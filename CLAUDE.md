...
Meta tags (title, description, OpenGraph, Twitter Card, `article:*`) are emitted by the partial `src/_includes/seo.njk`, included from `src/_includes/index.njk`. Site-level defaults (title, description, url, author, default OG image) live in `src/_data/site.js`. Per-page frontmatter overrides any of these.

- Page title is `frontmatter.title + " - " + site.title`; pages without `title` use `site.title` alone.
- `og:type` is `article` when `tags` includes `posts`, otherwise `website`.
- `og:image` is resolved from `frontmatter.image`. The partial **swaps a `.webp` extension for `.og.jpg`** because LinkedIn rejects WebP for `og:image` and times out on multi-MB originals. The `.og.jpg` companion file is generated by `pnpm og:gen` (see "Post image workflow" below).
- Pages without an `image` fall back to `site.defaultImage` (`/profile-pic.jpeg`).
- `image` accepts three forms: bare filename relative to the page (`butterfly.webp`), absolute path from web root (`/assets/profile-pic.jpeg`), or full URL (`https://...`).
...
- Located in `src/posts/` organized by `year/month/post-slug/index.md` (or `index.html`)
- Frontmatter: `title`, `excerpt`, `layout: post`, `tags: [posts, ...]`, `image`, `date`
- Post listing page: `src/posts/index.md`

#### Post image workflow

For posts with a hero image, the convention is **two derived files alongside the original**:

- `<name>.webp` - used for in-page rendering and the posts index card (`<img>` reads `frontmatter.image` directly).
- `<name>.og.jpg` - used **only** by social-media scrapers (LinkedIn, Facebook, Twitter). Resized to fit within 1200×630 at ~85% JPEG quality. Human visitors never see this file.

Frontmatter always points at the `.webp` (`image: cool-photo.webp`). The SEO partial swaps the extension to `.og.jpg` when emitting `og:image` / `twitter:image`.

**To add a hero image to a new post:**

1. Drop the original (`.jpg`, `.jpeg`, or `.png`) into the post folder.
2. Set `image: <name>.webp` in frontmatter (matching the original's stem).
3. Run `pnpm og:gen`.
4. Both `<name>.webp` and `<name>.og.jpg` appear next to the original. Commit all three.

The script (`scripts/generate-post-images.js`) reads each post's frontmatter, finds the source original by stem, and generates whichever derived files are missing or out of date (mtime check). It's idempotent and safe to re-run. For legacy posts that only have a `.webp` (no original), the script uses the `.webp` itself as the source for the `.og.jpg`.

`pnpm og:gen` is **manual** — not wired into `pnpm build` or `pnpm start`. Run it after adding or replacing a hero image.
