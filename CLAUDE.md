...
- RSS feed generation
- SEO plugin integration (handles page titles and meta tags)
- Auto-heading IDs with paperclip links (posts only)

### Page Titles
Page titles are generated by the `eleventy-plugin-seo` via the `{% seo "" %}` shortcode in `src/_includes/index.njk`. The plugin uses the page's `title` frontmatter when available, appending ` - Jschof.dev` as a suffix. Pages without a `title` frontmatter default to just `Jschof.dev`.

To set a page title, add `title` to the frontmatter:
```yaml
---
layout: index
title: "My Page Title"
